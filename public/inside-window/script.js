...
const interactiveFunctions = new Set([
...
const funcs = props.filter(p => typeof window[p] === 'function' && !interactiveFunctions.has(p));
let func = funcs[Math.floor(random() * funcs.length)];

function init() {
  func = funcs[Math.floor(random() * funcs.length)];
  ...
  features = {
    ...
    'Border': random() > 0.5 ? true : false,
    'Function': func,
    'Changing font size': random() > 0.1 ? true : false,
  };
...
async function draw() {
  ...
  try {
    const props = Object.getOwnPropertyNames(window)
    const funcs = props.filter(p => typeof window[p] === 'function')
    const func = funcs[Math.floor(random() * funcs.length)]
    window[func]()
  } catch (e) {
    // e.message
    try {
  ...
  async function drawBackgroundText() {
    const fontSizeDiff = features['Changing font size'] ? (1 + random() * 0.07 + 0.01) : 1;
...
window.onload = _event => {
  ...
  function startDrawing() {
    while (codeBoxDiv.firstChild) {
      codeBoxDiv.removeChild(codeBoxDiv.firstChild);
    }

    codeBoxDiv.style.backgroundColor = "";
    codeBoxDiv.style.color = "";

    draw();

    setTimeout(startDrawing, 20000);
  }
